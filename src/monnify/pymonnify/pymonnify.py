import base64, datetime, random, string, json
...
class InitializeMonnify:
    ...
    def __init__(self, contractCode: str, apiKey: str, secretKey: str, currencyCode: str = "", refStart: str = "",
    ...
    def __getAccesstoken(self) -> dict:
        ...
        if resp.status_code == 200:
            return {"accessToken": data['responseBody']['accessToken'], "message": data['responseMessage'],
                    "statusCode": resp.status_code, 'responseCode': data['responseCode']}
        return {"message": "Invalid credential, Unable to get token", "statusCode": resp.status_code,
    ...
    def __make_request(self, url: str, payload: dict = {}, method: str = "POST") -> dict:
        ...
        if accessToken['statusCode'] == 200:
            ...
            data = json.loads(resp.text)
            if resp.status_code == 200:
                return {"message": data['responseMessage'], "statusCode": resp.status_code,
                        'responseCode': data['responseCode'], "data": data['responseBody']}
            return {"message": data['responseMessage'], "statusCode": resp.status_code,
                    'responseCode': data['responseCode']}
        return {"message": accessToken['message'], "statusCode": accessToken['statusCode'],
                'responseCode': accessToken['responseCode']}
    ...
    def reserveBankAccount(self, customerName: str, customerEmail: str, customerBVN: str,
                           preferredCodes: list = []) -> dict:
        """
    ...
    def initializeTransaction(self, amount: str, customerName: str, paymentDescription: str, customerEmail: str,
                              redirectUrl: str) -> dict:
        """
